...
export const GRAPHITE_CONTEXT =
  'This is a Graphite repo. Use `gt` for branch creation, commit/amend, restack/sync, submit, and stack navigation. Submit any new PRs as drafts by default (`gt submit --draft ...`) to avoid requesting review of early iterations, and do not publish them unless the user explicitly asks or repo docs require it. Plain `git` is still appropriate for status/diff/staging and conflict resolution unless repo docs say otherwise. Load the graphite skill for the full command reference. Always provide explicit arguments and messages inline to avoid opening interactive prompts or an editor (e.g. `gt checkout <branch>` instead of bare `gt checkout`, `gt create -am "message"`, `gt submit --draft --no-edit`). Note: if the repo\'s AGENTS.md or project docs specify a different workflow, follow those instead.';
export const GITHUB_CONTEXT =
  'This repo uses standard GitHub PRs. Use `git` and `gh` for branching, pushing, and creating PRs. Create new PRs as drafts by default (`gh pr create --draft ...`) to avoid requesting review of early iterations, and do not mark them ready unless the user explicitly asks or repo docs require it. Load the github-workflow skill for best practices. Always provide explicit arguments and messages inline to avoid opening interactive prompts or an editor (e.g. `git commit -m "message"`, `gh pr edit --body-file ...`, `git rebase main`). Note: if the repo\'s AGENTS.md or project docs specify a different workflow, follow those instead.';
const GIT_CONTEXT =
